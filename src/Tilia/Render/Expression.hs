...
-- | Every shape an expression can take.
...
exprBody ctx site here = \case
  ...
  HsQual _ qs e ->
    at ctx qs (contextOf loneVariableExpr (hsExpr ctx) . fmap unbracketed)
      <> joinedBy "=>"
      <> hsExpr ctx e
...
tuple ctx here closing boxity args
  | any isMissing args = flat (brackets' (sepBy comma (fmap arg args)))
  | otherwise = brackets' (insideBrackets here (commaSep (fmap arg args)))
  where
...
arithSeq ctx closing = \case
  ...
  FromThenTo from next to ->
    wrap $
      commaSep (fmap (hsExpr ctx) [from, next])
        <> breakOrSpace
        <> txt ".."
        <> space
        <> hsExpr ctx to
...
-- | One field name written after a dot.
dotField :: Ctx -> DotFieldOcc GhcPs -> Doc
dotField ctx = name ctx . fmap (mkVarUnqual . field_label) . dfoLabel

-- | A run of field names, joined by the dots between them.
dotFields :: Ctx -> NonEmpty (DotFieldOcc GhcPs) -> Doc
dotFields ctx = sepBy (txt ".") . fmap (dotField ctx) . NE.toList
...
-- | Take an operator application apart, if that is what this is.
splitOpApp ::
  LHsExpr GhcPs ->
...
-- | What is inside, however many brackets were written around it.
...
-- | One statement of a block, a comprehension or a guard.
stmtBody ::
  ( Body b,
...
statements ctx site mkBody es =
  ...
  where
    ...
    item place stmt = case place of
      ...
      _ -> mayBraceWhenFlat (siteBracing site) rendered
      where
        rendered bracing =
    ...
    sections xs = case unsnoc xs of
      ...
      Just (stmts, yield) ->
        align (hsStmt ctx yield)
          <> breakOrSpace
          <> sepBy breakOrSpace (fmap section (comprehensionSections stmts))
    section stmts =
      located'
        (spansOf stmts)
        (txt "|" <> space <> align (commaSep (fmap (align . hsStmt ctx) stmts)))
...
comprehensionSections = fmap unnest . branches
  where
    branches = \case
...
localBinds ctx bracing = \case
    ...
    where
      sorted =
        sortBy
          ...
          (fmap (fmap Left) binds <> fmap (fmap Right) sigs)
      rendered place item = case place of
        Last -> render NoBrace
        Only -> render NoBrace
        _ -> mayBraceWhenFlat NoBrace render
        where
          render b =
  ...
  HsIPBinds ann (IPBinds _ xs) ->
    anchored ann (items bracing (fmap (at_ ctx implicitBind) xs))
...
-- | Every shape an arrow-notation command can take.
        ...
        <> includeUnless
          ...
          ( breakOrSpace
              <> indent
                ( sepBy
                    breakOrSpace
                    (fmap (printBody . CmdTopBody ctx plainSite) cmds)
                )
          )
...
-- | How the arrow of a command application is spelled.
  ...
  where
    ...
    head' = case pats of
      ...
      (headPat : tailPats) -> layoutFrom ctx headSpan $ case style of
        FunctionStyle n -> defHead (fromBool isInfix) (fromBool indentBody) (name ctx n) rendered
        ...
        LambdaCaseStyle ->
          hsPat ctx headPat
            <> includeUnless
              (null tailPats)
              (breakOrSpace <> indent (sepBy breakOrSpace (fmap (hsPat ctx) tailPats)))
      where
        rendered = fmap (hsPat ctx) pats
    ...
    body = sepBy breakOrSpace (fmap alternative (NE.toList grhssGRHSs))
...
-- | Does this pattern hold an or-pattern anywhere inside it?
  ...
  txt "pattern" <> case psb_args of
    PrefixCon args ->
      space
        <> name ctx psb_id
        <> indent
          ( layoutAcross ctx args (argsAfterName (fmap (name ctx) args) (null args))
...
untypedSplice ctx deco = \case
  HsUntypedSpliceExpr _ e -> spliceTH ctx (Isn't #typed) e deco
...
-- | A Template Haskell splice, typed or not.
spliceTH :: Ctx -> Choice "typed" -> LHsExpr GhcPs -> SpliceDecoration -> Doc
spliceTH ctx typed e = \case
  DollarSplice -> txt (if isTrue typed then "$$" else "$") <> spliced
  BareSplice -> spliced
  where
...
mayBraceWhenFlat :: Bracing -> (Bracing -> Doc) -> Doc
mayBraceWhenFlat whenBroken render =
  variant (render MayBrace) (render whenBroken)
...
bracedForSite site render
  | siteInBlock site && not (siteApplicand site) =
      mayBraceWhenFlat (siteBracing site) render
