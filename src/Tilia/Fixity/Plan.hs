...
remoteRepoCacheIn said = do
  spoken <-
    listToMaybe $
      reverse (filter (not . T.null) (map T.strip (T.lines said)))
...
-- enumerated without reading its reexports.
--
...
fromInterface interfaceOf modName =
  interfaceOf modName >>= \case
    Nothing -> pure (Unreadable Nothing)
    Just iface -> do
      declarers <- traverse asked (distinct (map fst (interfaceReexports iface)))
